...
| `deezx` | `192.168.1.161` | `/opt/deezx` | [deezx/docker-compose.yaml](deezx/docker-compose.yaml), [deezx/tool_chat_template_qwen3coder.jinja](deezx/tool_chat_template_qwen3coder.jinja) | `/root/.cache/huggingface` must be writable; the chat-template file must live beside the compose file |
...
| `deezx` | Research + retrieval node | `llama.cpp` CUDA + TEI CPU | Research chat: `Qwen/Qwen3.6-35B-A3B`; embeddings: `Qwen/Qwen3-Embedding-4B`; rerank: `BAAI/bge-reranker-v2-m3` | `32768` on research chat; not applicable on embedding/rerank | `8000`, `8001`, `8002` | Research spans both 3090s to keep the model at `Q8_0`; embeddings and rerank run on CPU TEI; the vendored Qwen tool template is required beside the compose file |
...
| `deezx` | Research chat | `Qwen/Qwen3.6-35B-A3B` | HF repo `bartowski/Qwen_Qwen3.6-35B-A3B-GGUF`, file `Qwen_Qwen3.6-35B-A3B-Q8_0.gguf` | `llama.cpp` CUDA | `32768` | Text chat, tool use |
| `deezx` | Embeddings | `Qwen/Qwen3-Embedding-4B` | `Qwen/Qwen3-Embedding-4B` | TEI CPU | Not applicable | Embeddings |
| `deezx` | Rerank | `BAAI/bge-reranker-v2-m3` | `BAAI/bge-reranker-v2-m3` | TEI CPU | Not applicable | Rerank |
...
flowchart LR
    ...
    Router --> Research[deezx\n192.168.1.161:8000\nQwen3.6 research via llama.cpp]
    Router --> Embed[deezx\n192.168.1.161:8001\nQwen3 embeddings]
    Router --> Rerank[deezx\n192.168.1.161:8002\nBGE reranker]
```
...
2. Restore the deployment directories: put each host bundle into its matching remote directory under `/opt`; `deezx` must include the vendored chat-template file, and `deezr` must include `config.yaml` beside the compose file.
...
- `deezx` keeps the research lane at `Q8_0` with `32768` context by splitting llama.cpp CUDA across both 3090s and moving embeddings plus rerank onto CPU TEI sidecars.
